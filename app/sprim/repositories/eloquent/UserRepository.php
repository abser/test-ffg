...
class UserRepository extends AbstractRepository implements UserInterface {
    ...
    public function createMember($input, $memberId, $user_id) {
        $clubUsersId = $this->ClubUsers->create(['user_id' => $memberId, 'club_id' => $input['club_id'],'created_by' => $user_id, 'type' => 1, 'status' => 1]);
    ...
    public function createUser($input, $memberId, $user_id) {
        ...
        $clubUsersId = $this->ClubUsers->create(['user_id' => $memberId, 'club_id' => $input['club_id'], 'type' => $input['user_type'],'created_by' => $user_id, 'status' => 1]);
        if (isset($input['member_phone'])) {
